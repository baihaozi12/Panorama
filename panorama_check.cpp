...
int generate_result(string img_name,int index,vector<Mat> full_imgs, vector<featuredata_pano> feature_data_list, vector<ImageFeatures> features,vector<MatchesInfo> pairwise_matches,result_return &result){
    Mat curr_frame = imread(img_name);
    full_imgs.push_back(curr_frame);
    // get features
    featuredata_pano result_feature_pano;
    get_combined_feature(result_feature_pano,curr_frame, index);
    feature_data_list.push_back(result_feature_pano);
    features.push_back(result_feature_pano.features) ;
    // match
    match_features(feature_data_list, pairwise_matches);
    //

    estimate_homo(feature_data_list, features, pairwise_matches, full_imgs,  index, result.result_img);
